...
class FIDEvaluation:
    ...
    def load_or_precalc_dataset_stats(self):
        ...
        try:
            ...
            for _ in tqdm(range(num_batches)):
                try:
                    samples = next(self.dl)
                except StopIteration:
                    break
                real_samples = samples['B'].to(self.device)
    ...
    def fid_score(self):
        ...
        for batch in tqdm(batches):
            samples = next(self.dl)
            if self.dataset == 'tissue':
                data_cy3 = samples['cy3'].to(self.device)
                data_dapi = samples['dapi'].to(self.device)
                data_A = torch.cat((data_cy3,data_dapi),dim=1)
            else:
                data_A = samples['A'].to(self.device)    
            fake_samples = self.sampler.sample(x_input_cond=data_A, batch_size=batch)
